/***************************************************************************************
*    Title: GodotSharp/GodotSharp/Core/Vector2.cs
*    Author: GodotEngine
*    Date: 04/08/2025
*    Code version: 4.4.1
*    Availability: https://github.com/raulsntos/godot/blob/master/modules/mono/glue/GodotSharp/GodotSharp/Core/Vector2.cs
***************************************************************************************/
...
namespace Repl.Server.Core.MathUtils;
    ...
    public struct Vector2 : IEquatable<Vector2>
    {
        ...
        {
            return MathF.Abs(LengthSquared() - 1.0f) < MathExtension.EPSILON;
        }
        ...
        public readonly Vector2 Lerp(Vector2 to, float weight)
        {
            return new Vector2
            (
                MathExtension.Lerp(X, to.X, weight),
                MathExtension.Lerp(Y, to.Y, weight)
            );
        }
        ...
        public readonly Vector2 MoveToward(Vector2 to, float delta)
        {
            ...
            Vector2 vd = to - v;
            float len = vd.Length();
            if (len <= delta || len < MathExtension.EPSILON)
                return to;
        ...
        {
            Vector2 v = new Vector2(MathF.Sign(X), MathF.Sign(Y));
            return v;
        }
        ...
        public readonly Vector2 Slerp(Vector2 to, float weight)
        {
            ...
            float resultLength = MathExtension.Lerp(startLength, MathF.Sqrt(endLengthSquared), weight);
        ...
        {
            return MathExtension.IsEqualApprox(X, other.X) && MathExtension.IsEqualApprox(Y, other.Y);
        }
        ...
        {
            return MathExtension.IsZeroApprox(X) && MathExtension.IsZeroApprox(Y);
        }
